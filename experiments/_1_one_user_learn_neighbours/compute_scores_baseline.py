...
import json
from fit_baseline_models import convert_to_popularity, load_Xallpop

MODELS_FOLDER = "/media/pablo/data/Tesis/models/old/"


def load_model_small(uid):
    model_path = join(MODELS_FOLDER, "svcpop_%d_small_.pickle" % uid)
    clf = joblib.load(model_path)

    return clf

def worker(uid, Xallpop, f1s_train, f1s_valid, f1s_testv, precisions_train, precisions_valid, precisions_testv,
    ...
    lock.acquire()    
    X_train = convert_to_popularity(X_train, Xallpop)
    X_valid = convert_to_popularity(X_valid, Xallpop)
    X_testv = convert_to_popularity(X_testv, Xallpop)
    lock.release()    
...
if __name__ == '__main__':
    ...
    for uid in uids:
        pool.apply_async(worker, (uid, Xallpop, f1s_train, f1s_valid, f1s_testv,
    ...
    with open('scores/baseline/f1s_train_svc.json', 'w') as f:
        json.dump(dict(f1s_valid), f)

    with open('scores/baseline/f1s_valid_svc.json', 'w') as f:
        json.dump(dict(f1s_valid), f)

    with open('scores/baseline/f1s_testv_svc.json', 'w') as f:
        json.dump(dict(f1s_testv), f)

    with open('scores/baseline/precisions_train_svc.json', 'w') as f:
        json.dump(dict(precisions_valid), f)

    with open('scores/baseline/precisions_valid_svc.json', 'w') as f:
        json.dump(dict(precisions_valid), f)

    with open('scores/baseline/precisions_testv_svc.json', 'w') as f:
        json.dump(dict(precisions_testv), f)

    with open('scores/baseline/recalls_train_svc.json', 'w') as f:
        json.dump(dict(recalls_valid), f)

    with open('scores/baseline/recalls_valid_svc.json', 'w') as f:
        json.dump(dict(recalls_valid), f)

    with open('scores/baseline/recalls_testv_svc.json', 'w') as f:
