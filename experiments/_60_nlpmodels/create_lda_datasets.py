...
def load_all_f1s():
    with open('../_1_one_user_learn_neighbours/f1s_valid.json') as f:
        f1s = json.load(f)
...
def load_nlp_selected_users():
    if not exists('nlp_users.json'):
        ...
        # Nos quedamos con los de valid f1 < 70% ( es un proceso pesado )
        low_ur = [(u,r) for (u,r) in ur if r < 0.7]

        high_ur = [(u,r) for (u,r) in ur if r >= 0.7 and r < 1]
