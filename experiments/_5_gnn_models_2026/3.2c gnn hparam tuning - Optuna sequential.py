...
MAX_EVALS = 10  # Total number of Optuna trials
...
# Pre-trained weights initialization (set to a .pt file path to warm-start)
# Example: "./experiments/v3_full_N300/best_retweet_gnn_general.pt"
# INIT_WEIGHTS_PATH = None  # None = train from scratch
INIT_WEIGHTS_PATH = "./experiments/v2_N20/best_retweet_gnn_general.pt"  # None = train from scratch

FINETUNE_LR_FACTOR = 0.1  # When fine-tuning, multiply Optuna-suggested LR by this factor
FINETUNE_WD_FACTOR = 0.2  # When fine-tuning, multiply weight_decay by this factor
FINETUNE_WARMUP_EPOCHS = 2  # Override warmup_epochs when fine-tuning

# Output directory for tuning results
TUNING_DIR = f"./experiments/{FINAL_TAG}_hparam_tuning_seq"
os.makedirs(TUNING_DIR, exist_ok=True)

print(f"Base experiment: {FINAL_TAG}")
print(f"Tuning output:  {TUNING_DIR}")
print(f"Trials: {MAX_EVALS} (sequential, single node)")
print(f"Epochs per trial: {EPOCHS}")
if INIT_WEIGHTS_PATH:
    print(f"Fine-tuning from: {INIT_WEIGHTS_PATH}")
    print(f"  LR factor={FINETUNE_LR_FACTOR}, WD factor={FINETUNE_WD_FACTOR}, warmup={FINETUNE_WARMUP_EPOCHS}")
else:
...
def objective(trial):
    ...
    Runs sequentially on the driver GPU.
    ...
    drop_edge_rate = trial.suggest_float("drop_edge_rate", 0.0, 0.3)
    warmup_epochs = trial.suggest_categorical("warmup_epochs", [2, 3, 5, 8])
    grad_accum_steps = trial.suggest_categorical("grad_accum_steps", [2, 4, 8, 16])
    lr_schedule = trial.suggest_categorical("lr_schedule", ["cosine", "linear"])

    # Apply fine-tune factors when using pre-trained weights
    if INIT_WEIGHTS_PATH:
        lr *= FINETUNE_LR_FACTOR
        weight_decay *= FINETUNE_WD_FACTOR
        warmup_epochs = FINETUNE_WARMUP_EPOCHS

    # Select GPU
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    # Each trial gets its own nested MLflow run
    with mlflow.start_run(nested=True):
        # Log hyperparameters upfront (available even if trial crashes)
        mlflow.log_params({
            "lr": lr,
            ...
            "init_weights_path": str(INIT_WEIGHTS_PATH),
            "finetune": bool(INIT_WEIGHTS_PATH),
        })
        try:
            # Build model
            model = RetweetGNN(
            ...
            # Load pre-trained weights or initialize gate
            if INIT_WEIGHTS_PATH:
            ...
            for epoch in range(EPOCHS):
                ...
                for chunk_start in range(0, len(users), TRAIN_CHUNK_SIZE):
                    ...
                    for start in range(0, len(chunk_data_list) - batch_size + 1, batch_size):
                        ...
                        epoch_loss_count += 1
                        epoch_train_preds.extend(out.argmax(dim=1).detach().cpu().tolist())
                        epoch_train_labels.extend(batch.y.cpu().tolist())
                        accum_count += 1
                    ...
                    del chunk_data_list
                    del batch  # free last batch GPU reference
                    gc.collect()
                ...
                del batch  # free last validation batch from GPU
                torch.cuda.empty_cache()

                val_f1 = f1_score(all_labels, all_preds)
                train_f1 = f1_score(epoch_train_labels, epoch_train_preds) if epoch_train_labels else 0.0
                ...
                history.append({"epoch": epoch, "train_loss": avg_train_loss, "val_loss": avg_val_loss, "train_f1": train_f1, "val_f1": val_f1})
            ...
            mlflow.log_text(json.dumps(history, indent=2), "training_history.json")
            ...
            print(f"  Trial {trial.number}: val_f1={best_val_f1:.4f} ({elapsed:.0f}s)")

            # Cleanup: aggressively free GPU + CPU memory between trials
            del model, optimizer, scheduler, criterion
            del val_batches, all_preds, all_labels, history
            try:
                del batch
            except NameError:
                pass
            ...
            for var in ['model', 'optimizer', 'scheduler', 'criterion',
                        'val_batches', 'all_preds', 'all_labels', 'history', 'batch']:
                try:
                    del locals()[var]
...
# DBTITLE 1,Run Optuna sequential study
...
print(f"Starting Optuna search: {MAX_EVALS} trials (sequential)")
...
with mlflow.start_run(run_name="gnn_hparam_tuning_optuna_seq") as run:
    experiment_id = run.info.experiment_id

    study_name = "gnn-hparam-tuning-seq"
    ...
    if RESTART_TUNING:
        ...
        # Resume existing study (in-memory only; for persistent resume use a DB storage)
        study = optuna.create_study(
            study_name=study_name,
            direction="minimize",
            sampler=optuna.samplers.TPESampler(seed=42),
        )
        print(f"Created study (in-memory; sequential mode does not persist across restarts).")
    ...
    mlflow.log_metrics({"best_val_f1": -study.best_value})
    mlflow.log_params({f"best_{k}": v for k, v in study.best_params.items()})

# COMMAND ----------

# DBTITLE 1,Display failed trial errors
# Show errors from failed trials
...
ax.set_title('Optuna Convergence (Sequential)')
ax.legend()
...
if INIT_WEIGHTS_PATH:
    ...
    if unexpected:
        print(f"  Unexpected keys (ignored): {unexpected}")
    print(f"  Pre-trained weights loaded. gate_param = {model.gate_param.item():.4f}")
else:
    with torch.no_grad():
        model.gate_param.fill_(0.0)

# Apply fine-tune factors (same as in objective)
_retrain_lr = best_params["lr"] * FINETUNE_LR_FACTOR if INIT_WEIGHTS_PATH else best_params["lr"]
_retrain_wd = best_params["weight_decay"] * FINETUNE_WD_FACTOR if INIT_WEIGHTS_PATH else best_params["weight_decay"]
_retrain_warmup = FINETUNE_WARMUP_EPOCHS if INIT_WEIGHTS_PATH else best_params["warmup_epochs"]

print(f"Retraining best model with:")
for k, v in sorted(best_params.items()):
    print(f"  {k}: {v}")
if INIT_WEIGHTS_PATH:
    print(f"\n  (Fine-tune effective: lr={_retrain_lr:.2e}, wd={_retrain_wd:.2e}, warmup={_retrain_warmup})")
print(f"\nDevice: {device}")
...
model, history = train_model(
    model=model,
    train_loader=train_loader if 'train_loader' in dir() else None,
    ...
    device=device,
    lr=_retrain_lr,
    weight_decay=_retrain_wd,
    lr_warmup_epochs=_retrain_warmup,
